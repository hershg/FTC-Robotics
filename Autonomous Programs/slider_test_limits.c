  ...
  while (true)
  {
	    ...
	    if(nNxtButtonPressed == 2)
	    {
	      if( (HTTMUXisActive(HTTMUX, SlideLift_Touch) == false)  || 	// sensor not touched
	      	  (nMotorEncoder[SlideLift] > 2000 )	)											// sensor touched -  at top limit so check encoder value..
		    	motor[SlideLift] = -100;
	    	else
	    	{
    			motor[SlideLift] = 0;
	  	...
	  	{
	     	if ((HTTMUXisActive(HTTMUX, SlideLift_Touch) == false) || // sensor not touched
	    		  (nMotorEncoder[SlideLift] < 2000)) 							// sensor touched but at bottom limit so check encoder value
	    		motor[SlideLift] = 100;
	     	else
		    	motor[SlideLift] = 0;
	  	}
	    ...
	    if(HTTMUXisActive(HTTMUX, SlideLift_Touch))
	    	nxtDisplayTextLine(1, "SlideLift Pressed");
	   	else
			 	nxtDisplayTextLine(1, "SlideLift Not");

			if(HTTMUXisActive(HTTMUX, ArticArm_Touch))
	    	nxtDisplayTextLine(3, "ArticArm Pressed");
	   	else
			 	nxtDisplayTextLine(3, "ArticArm Not");

			nxtDisplayTextLine(5, "position = %d", nMotorEncoder[SlideLift]);
  }
}
